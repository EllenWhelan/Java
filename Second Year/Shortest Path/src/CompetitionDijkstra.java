...
public class CompetitionDijkstra {
        ...
        if(graph!=null) {
            ...
            for (int i = 0; i < graph.vertices; i++) {
                ...
                for (int j = 0; j < distances.length; j++) {
                    if (distances[j] > longestDist) longestDist = distances[j];
                    if (distances[j] == Float.POSITIVE_INFINITY&& i!=j) return -1;
                }
            }
            return longestDist;
        }
   ...
   int findSlowestWalkerDij(int sA, int sB, int sC){
    ...
    /*public static void main(String [] args){
       CompetitionDijkstra newG= new CompetitionDijkstra("input-I.txt", 60, 70, 84);
